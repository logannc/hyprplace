...
| Lua | 5.5, for the installer and the CLI tools |
| hyprsplit | **Not** required. Works with or without it |
...
Only set those once the extension is installed; with no tag to wait for, deferral just
delays the same decision. On timeout the window is placed exactly as it would have been
...
Working, and in use. This is a v0: option names and defaults may still change, and the
Firefox extension is newer than the plugin. 555 unit tests run without a compositor
(`make test`).
